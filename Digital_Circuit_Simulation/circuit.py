...
class PriorityQueue:
    """ Priority queue implementation.
        Min heap data structure implemented with array.
    """
    ...
    def leftChild(self, index):
        """ Returns left child of node at given index or None if not exist. """

        if index*2+1 < self.queue_length:
            return self.queue[2*index+1]
        return None

    def rightChild(self, index):
        """ Returns right child of node at given index or None if not exist. """

        if index*2+2 < self.queue_length:
            return self.queue[2*index+2]
        return None

    def heapNode(self, index):
        """ Returns node at given index or None if not exist. """
        """ Specifics of building heap structure on array is that
            nodes populate first half of array and
            leafs populate second half of array
        """

        if index< self.queue_length//2:
            return self.queue[index]
        return None

    def heapSwap(self, i, left = True):
        """ Swaps node and node's child values
            according to index and direction
        """
        if left:
            self.queue[2*i+1], self.queue[i] = self.queue[i], self.queue[2*i+1]
            # print "Swap" + repr(self.queue[2*i+1]) + repr(self.queue[i])
        else:
            self.queue[2*i+2], self.queue[i] = self.queue[i], self.queue[2*i+2]
            # print "Swap" + repr(self.queue[2*i+2]) + repr(self.queue[i])


    def heapNodeChildCount(self, i=0):
        """ Returns how many childs given node has.
        """
    ...
    def heapPush(self, i):
        """ Recursive function.
            Pushes node recursively down the heap until
            position with no heap violation reached
        """

        if self.heapNodeChildCount(i) == 1 and self.leftChild(i) < self.heapNode(i):
            self.heapSwap(i, True)
            self.heapPush(2*i+1)
        if self.heapNodeChildCount(i) == 2:
            if self.leftChild(i) < self.heapNode(i):
                if self.leftChild(i) <= self.rightChild(i):
                    self.heapSwap(i, True)
                    self.heapPush(2*i+1)
                else:
                    self.heapSwap(i, False)
                    self.heapPush(2*i+2)
            if self.rightChild(i) < self.heapNode(i):
                self.heapSwap(i, False)
                self.heapPush(2*i+2)


    def checkAndCorrectHeapViolation(self, i):
        """ Recursive function.
            Perform swap if structure violation found,
            then recursively check parent for violation.
            Recursion moves up the structure until root reached.
        """

        if self.leftChild(i) != None and self.heapNode(i) > self.leftChild(i):
            self.heapSwap(i, True)

        if self.rightChild(i) != None and self.heapNode(i) > self.rightChild(i):
            self.heapSwap(i, False)

        if i>0:
            if i%2 > 0:
                self.checkAndCorrectHeapViolation(i//2)
            else:
                self.checkAndCorrectHeapViolation(i//2 - 1)

    def append(self, key):
        """ Appends a node to the end of heap.
            Then start heap violation check from this node.
        """
        if key is None:
        ...
        self.checkAndCorrectHeapViolation(self.queue_length-1)

    def min(self):
        """ Returns min node in heap.
            In case of min heap structure it is root node.
        """
    ...
    def pop(self):
        ...
        self.queue[0], self.queue[self.queue_length-1] = self.queue[self.queue_length-1], self.queue[0]

        # cut heap's last leaf end. push root to leafs
        self.queue = self.queue[0:self.queue_length-1]
        self.queue_length = len(self.queue)
        self.heapPush(0)
