...
class CrossVerifier(object):
  ...
  def __init__(self, layer):
    ...
    self.horizontal = []
    self.horizontalLines = AVLTree()
    # self.vertical = []
    self.verticalLines = AVLTree()
    ...
    #   else:
    #     arr[len(arr)-1][1] = arr[len(arr)-1][1] + self.horizontal[i][1]
  ...
  def _events_from_layer(self, layer):
    ...
    for wire in layer.wires.values():                            ### o(n)
      if wire.is_horizontal():
        self.events.append([wire.y1, 0, wire.object_id, 'add', wire])
        self.horizontalLines.insert(Node(wire.y1, wire))
      else:
        self.events.append([wire.x1, 1, wire.object_id, 'query', wire])
        self.verticalLines.insert(Node(wire.x1, wire))
...
###
    ...
    for wire in layer.wires.values():                            ### o(n)
      ...
      if wire.is_horizontal():
        # self.events.append([wire.y1, 0, wire.object_id, 'add', wire])
        self.horizontalLines.insert(Node(wire.y1, KeyWirePair(wire.x2, wire)))
        counterH += 1
      else:
        # self.events.append([wire.x1, 1, wire.object_id, 'query', wire])
        self.verticalLines.insert(Node(wire.x1, KeyWirePair(wire.y2, wire)))
        counterL += 1

      result += self.computeSingleWireCrossings(wire, count_only)
###
###
    ...
    #     upperBound = self.horizontalLines.findByKey(wire.y2, False, True)

    #     selectedNode = lowerBound
    #     while lowerBound <= upperBound and selectedNode and selectedNode <= upperBound:
    ...
    #         # counter += 1
    #       selectedNode = self.horizontalLines.nextNode(selectedNode)
    ...
    print "H: " + str(counterH) + " L: " + str(counterL)
    return result

  def bSearch(self, array, key):
    if array[-1].key < key:
      return None
    if len(array) == 1:
      return [0]
    ...
    while top-bottom>1:
      currIndex = (top+bottom)//2
      candidate = array[currIndex]
      if candidate.key == key:
        return [currIndex, currIndex + 1]
      if candidate.key < key:
        bottom = currIndex
      else:
        top = currIndex
  ...
  def computeSingleWireCrossings(self, wire, count_only):
    if count_only:
      result = 0
    else:
      result = self.result_set
    ...
    if wire.is_horizontal():
      tree = self.verticalLines
      lowerBound = tree.findByKey(wire.x1, True)
      upperBound = tree.findByKey(wire.x2, False, True)
      key = wire.y1
    else:
      tree = self.horizontalLines
      lowerBound = tree.findByKey(wire.y1, True)
      upperBound = tree.findByKey(wire.y2, False, True)
      key = wire.x1
    ...
    if lowerBound and upperBound and lowerBound <= upperBound:
      ...
      while selectedNode and selectedNode <= upperBound:
        counterWhile += 1
        # # crossTree = AVLTree()
        # for potentialCrossWire in selectedNode.value:
        #   # print potentialCrossWire
        ...
        #     cross_wires.append(potentialCrossWire)
          # counter += 1
        potentialCrossWires = selectedNode.value
        indexOfInterest = self.bSearch(selectedNode.value, key)

        # print selectedNode
        # print potentialCrossWires
        # print indexOfInterest

        if indexOfInterest:
          for i in indexOfInterest:
            if wire.intersects(potentialCrossWires[i].wire):
              cross_wires.append(potentialCrossWires[i].wire)


        selectedNode = tree.nextNode(selectedNode)
      print "counter while: " + str(counterWhile)
    if count_only:
      result += len(cross_wires)
    else:
      for cross_wire in cross_wires:
