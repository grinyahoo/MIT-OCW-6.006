...
class RangeIndex(object):
  ...
  def list(self, node, lowerBound, upperBound, result = []):
    """List of values for the keys that fall within [first_key, last_key]."""
    if node is None:
      return []
    if lowerBound<=node and node<=upperBound:
      result += node.value
    if node>=lowerBound:
      result += self.list(node.left, lowerBound, upperBound, result)
    if node<=upperBound:
      result += self.list(node.right, lowerBound, upperBound, result)
    return result
  
  # def count(self, lowerBound, upperBound):
  #   """Number of keys that fall within [first_key, last_key]."""  
  #   return len(self.list(lowerBound, upperBound))

  def range(self, lowerBound, upperBound):
    if lowerBound is None or upperBound is None or lowerBound > upperBound:
      return 0
    if lowerBound == upperBound:
      return len(lowerBound.value)
    else:
      mca = self.treeY.minCommonAncestor(lowerBound, upperBound)
      # print "mca: " + repr(mca)
      # print mca
      r = mca.rank

      print "rank minCommonAncestor: " + str(r)
      if lowerBound.left:
        r -= lowerBound.left.rank
        print "rank sub left: " + str(r)
      if upperBound.right:
        r -= upperBound.right.rank
        print "rank sub right: " + str(r)
...
class CrossVerifier(object):
  ...
  def _events_from_layer(self, layer):
    ...
    for wire in layer.wires.values():
      if wire.is_horizontal():
        ...
        self.events.append([wire.x2, 0, wire.object_id, 'delete', wire])
      else:
  ...
  def _compute_crossings(self, count_only):
    ...
    tree = self.index.treeY

    prevEvent_x = self.events[0][0]
    for event in self.events:
      event_x, event_type, wire = event[0], event[3], event[4]
      self.trace_sweep_line(event_x)

      if prevEvent_x < event_x:
        for w in self.deleteList:
          node = tree.findByKey(w.y1)
          tree.delete(node, w)
        self.deleteList = []
        
      if event_type == 'delete':
        self.deleteList.append(wire)
        # print "del " + repr(wire)

      if event_type == 'add':

        yNode = Node(wire.y1, wire)
        tree.insert(yNode)
      ...
      if event_type == 'query':
        ...
        # print "====="
          ...
          if lowerBound and upperBound and lowerBound<=upperBound:
            # mca = tree.minCommonAncestor(lowerBound, upperBound)
            # nodeList = self.index.list(mca, lowerBound, upperBound)
            # result = len(nodeList)
            result += tree.rangeQty(lowerBound, upperBound)

        else:
          # result = self.result_set
          cross_wires = tree.rangeList(lowerBound,upperBound)
          for w in cross_wires:
            result.add_crossing(wire, w)
        ...
        #         result.add_crossing(wire, potentialCrossWire)
