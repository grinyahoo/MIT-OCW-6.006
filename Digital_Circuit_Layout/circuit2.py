...
class CrossVerifier(object):
  ...
  def __init__(self, layer):
    ...
    # print repr(self.horizontal)

    # self.horizontalLines = BSTree(self.horizontal)
  ...
  def _compute_crossings(self, count_only):
    ...
    #     # print "Lower: " + repr(lowerBound)
    #     # print "Upper: " + repr(upperBound)
    ...
    for event in self.events:
      ...
      # print repr(event) + "\n"
      self.trace_sweep_line(event_x)
      for indexedWire in self.index.data:
        if indexedWire.wire.x2 < event_x:
          self.index.remove(indexedWire)

        # print indexedWire.wire.x2 < event_x
      ...
      if event_type == 'query':
        ...
        # for e in self.events:
          # e_type, e_wire = e[3], e[4]
          # if e_type == 'add' and e_wire.x1 <= event_x <= e_wire.x2:
          #   self.index.add(KeyWirePair(e_wire.y1, e_wire))
